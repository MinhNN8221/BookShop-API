...
export default {
  ...
  createToken(user) {
    ...
    return jwt.sign({ customer_id: customerId, name, email }, process.env.SECRET, { expiresIn: 1296000 });
  },
  ...
  validateChangePass(user) {
    return Joi.validate(user, schema.changePassSchema, options);
  },

  validateForgotPass(user) {
    return Joi.validate(user, schema.forgotPassSchema, options);
  },
  ...
  validateItemCartDetails(user) {
    return Joi.validate(user, schema.shoppingItemCartSchema, options);
  },
  validateOrderDetails(user) {
    return Joi.validate(user, schema.orderSchema, options);
  },
  validateWishlistDetails(user) {
    return Joi.validate(user, schema.wishlistSchema, options);
  },
